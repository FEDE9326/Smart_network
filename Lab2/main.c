...
double lambda[2]={LAMBDA_L LAMBDA_H};
double mu[2]={C_L C_H};
int total_users;
...
{ Time delta;
  Record *rec;
  int c=change_status(LIMIT);
  delta = negexp(1.0/lambda[status],&seme);
  ...
  if (in_service == NULL)
    {
      ...
      /*se cambio ci metto un po di piu*/
      if(c==1)
      current_time+=switch_time[!status];

      schedule(DEPARTURE,current_time+negexp(1/mu[status],&seme));
    }
  else
    in_list(&queue,rec);
  return;
  ...
  if (queue!=NULL)
   {
    ...
    schedule(DEPARTURE,current_time+negexp(1.0/mu[status],&seme));
   }
  return;
}
...
int change_status(int limit){

    double val=uniform(0,100,seme);
    if(val>=limit){
        status=!status;
        schedule(SWITCH,current_time);
        return 1;
...
/*fare versione schedulata uno scheduling di change tira l'altro*/
 change_status(50);
 schedule(ARRIVAL,negexp(1.0/lambda[status],&seme));
